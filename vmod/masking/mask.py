...
class nwtTiDEMasks():
    ...
    def maskAll(self, threshold=0.2, window_size=200, mask_type="factor"):
        ...
        components = list(self.cov.components) + list(self.tgt.components)

        for start in range(0, n_time, window_size):
            end = min(start + window_size, n_time - 1)

            for comp in components:
                if comp in self.cov.components:
                    target_attr = 'cov'
                elif comp in self.tgt.components:
                    target_attr = 'tgt'
                else:
                    raise ValueError(f"Component {comp} not found in covariates or target time series.")
                ...
                if np.random.rand() < threshold:
                    ...
                    if mask_type == "factor":
                        ...
                        setattr(self, target_attr, masked)
    ...
    def maskPhaseConstant(self, components, phaseStep=5):
        """
        Applies a phase shift to the specified components by a constant number of steps.
        Only modifies self.tgt or self.cov if relevant components are shifted.
        """
        if phaseStep == 0:
            return
        ...
        shifted_tgt = arr_tgt.copy()
        shifted_cov = arr_cov.copy()

        tgt_modified = False
        cov_modified = False

        for comp in components:
            if comp in self.tgt.components:
                comp_idx = self.tgt.components.get_loc(comp)
                if phaseStep > 0:
                    shifted_tgt[phaseStep:, comp_idx] = arr_tgt[:-phaseStep, comp_idx]
                else:
                    shifted_tgt[:phaseStep, comp_idx] = arr_tgt[-phaseStep:, comp_idx]
                tgt_modified = True

            elif comp in self.cov.components:
                comp_idx = self.cov.components.get_loc(comp)
                if phaseStep > 0:
                    shifted_cov[phaseStep:, comp_idx] = arr_cov[:-phaseStep, comp_idx]
                else:
                    shifted_cov[:phaseStep, comp_idx] = arr_cov[-phaseStep:, comp_idx]
                cov_modified = True
...
@staticmethod
def ramp_mask_ts(ts, components, ind=None, ramp_duration=10):
    """
    Applies a ramped mask to the specified components of a TimeSeries,
    replacing values from ind[0] to ind[1] with a ramp from 1.0 to 0.0
    multiplied by the value at ind[0]. Values after the ramp within the
    mask window are set to zero.

    Parameters:
    - ts: TimeSeries object to be masked.
    - components: List of component names to be masked.
    - ind: [start_index, end_index] defining mask window. Defaults to full series.
    - ramp_duration: Number of time steps to ramp from 1.0 to 0.0 (default: 60).
    ...
    # Define full ramp from 1.0 to 0.0
    full_ramp = np.linspace(1.0, 0.0, ramp_duration)
    ramp = full_ramp[:window_length]  # Truncate if window shorter

    for c_idx in comp_indices:
        if start > 0:
            base_val = values[start - 1, c_idx]
        else:
            base_val = 0.0
        # Apply ramped values
        values[start:start + len(ramp), c_idx] = base_val * ramp
        # Fill remaining window with zeros (if any)
        if len(ramp) < window_length:
            values[start + len(ramp):end, c_idx] = 0.0
