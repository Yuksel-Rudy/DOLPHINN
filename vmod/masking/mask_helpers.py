...
def uncertainty(ts, quantiles=[0.025, 0.975], sigma=3.0):
    ...
    # Variable-based smoothing
    # delta_smooth = gaussian_filter1d(delta, sigma=sigma)
    
    # Time-based smoothing
    delta_smooth = np.zeros_like(delta)
    for i in range(delta.shape[1]):
        delta_smooth[:, i] = gaussian_filter1d(delta[:, i], sigma=sigma)
    return delta, delta_smooth

def get_predictionHorizon(ts, delta_0='solve_for', tau_max=60, plot=False):
    """
    ...
    if plot:
        ...
        n_points = len(ts.time_index)
        x_normalized = np.linspace(0, 1, n_points)
        fig, ax = plt.subplots(1, 1, figsize=(10, 5))
        n_components = delta.shape[1]
        ...
        ax.axhline(delta_0, color='black', linestyle='--', label=r'$\delta_0$ - mean')
        ax.set_ylabel(r'$\delta$ (m)')
        ax.set_xlabel(r"$\tau/\tau_{\mathrm{max}}$")
...
def rolling_probabilistic_forecast(
    ...
    for i, s in enumerate(strides):
        ...
        taus, _ = get_predictionHorizon(forecast_scaler, delta_0=delta_0, plot=True)
        plt.show()
        allTaus[i, :] = taus
