...
# Mask settings
prbN = 7  # number of probes to compute MAE for in the end
...
n_repeat = 25
mae_store = np.zeros((n_repeat, prbN-1, len(rhos)))
...
for r in range(n_repeat):
    for i, rho in enumerate(rhos):
        ...
        backtest_scaled = model.historical_forecasts(
            mask.tgt,
            forecast_horizon=n,  # n, int(33 / dt)
            verbose=True,
            retrain=False
        )
        ...
        mae_values = {}
        for comp in val.components[-prbN:-1]:
...
probes = val.components[-prbN:-1]
mae_df = pd.DataFrame(mae_mean, columns=rhos, index=probes)
ci_df  = pd.DataFrame(ci95,  columns=rhos, index=probes)

mae_df.to_csv(os.path.join(filePath, f"CI_LSTM_MAE_mean_dlta.csv"))
ci_df.to_csv(os.path.join(filePath, f"CI_LSTM_MAE_ci95_dlta.csv"))
